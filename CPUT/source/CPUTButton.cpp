...
// Constructor
//-----------------------------------------------------------------------------
CPUTButton::CPUTButton(const std::string ControlText, CPUTControlID id, CPUTFont *pFont):
...
CPUTButton* CPUTButton::Create(std::string const& ControlText, CPUTControlID id, CPUTFont* pFont) {
	return new CPUTButton(ControlText, id, pFont);
}
...
//--------------------------------------------------------------------------------
CPUTEventHandledCode CPUTButton::HandleKeyboardEvent(CPUTKey key, CPUTKeyState state)
{
...
//--------------------------------------------------------------------------------
void CPUTButton::GetText(std::string &String)
{
    if(mpButtonText)
    {
...
void CPUTButton::SetPosition(int x, int y)
{
    ...
    if(mpButtonText)
    {
        ...
        CPUT_RECT rect;
        mpButtonText->GetDimensions(rect.width, rect.height);
		//if (rect.width > mButtonDimensions.width || rect.height > mButtonDimensions.height)
		{
			Resize(rect.width, rect.height);
		}
...
CPUTResult CPUTButton::RegisterStaticResources()
{
    ...
    mSmallestLeftSizeIdle = std::min(std::min(mpButtonIdleImageSizeList[0].width, mpButtonIdleImageSizeList[1].width), mpButtonIdleImageSizeList[2].width);
    mSmallestLeftSizePressed = std::min(std::min(mpButtonPressedImageSizeList[0].width, mpButtonPressedImageSizeList[1].width), mpButtonPressedImageSizeList[2].width);
    mSmallestLeftSizeDisabled = std::min(std::min(mpButtonDisabledImageSizeList[0].width, mpButtonDisabledImageSizeList[1].width), mpButtonDisabledImageSizeList[2].width);  
    
    // find the narrowest (width) right side images
    mSmallestRightSizeIdle = std::min(std::min(mpButtonIdleImageSizeList[6].width, mpButtonIdleImageSizeList[7].width), mpButtonIdleImageSizeList[8].width);
    mSmallestRightSizePressed = std::min(std::min(mpButtonPressedImageSizeList[6].width, mpButtonPressedImageSizeList[7].width), mpButtonPressedImageSizeList[8].width);
    mSmallestRightSizeDisabled = std::min(std::min(mpButtonDisabledImageSizeList[6].width, mpButtonDisabledImageSizeList[7].width), mpButtonDisabledImageSizeList[8].width);  
    

    // find the shortest (height) of the top row of images
    mSmallestTopSizeIdle = std::min(std::min( mpButtonIdleImageSizeList[0].height,mpButtonIdleImageSizeList[3].height), mpButtonIdleImageSizeList[6].height);
    mSmallestTopSizePressed = std::min(std::min( mpButtonPressedImageSizeList[0].height,mpButtonPressedImageSizeList[3].height), mpButtonPressedImageSizeList[6].height);
    mSmallestTopSizeDisabled = std::min( std::min( mpButtonDisabledImageSizeList[0].height,mpButtonDisabledImageSizeList[3].height), mpButtonDisabledImageSizeList[6].height);

    // find the shortest (height) of the bottom row of images
    mSmallestBottomSizeIdle = std::min(std::min( mpButtonIdleImageSizeList[2].height,mpButtonIdleImageSizeList[5].height), mpButtonIdleImageSizeList[8].height);
    mSmallestBottomSizePressed = std::min(std::min( mpButtonPressedImageSizeList[2].height,mpButtonPressedImageSizeList[5].height), mpButtonPressedImageSizeList[8].height);
    mSmallestBottomSizeDisabled = std::min(std::min( mpButtonDisabledImageSizeList[2].height,mpButtonDisabledImageSizeList[5].height), mpButtonDisabledImageSizeList[8].height);
...
CPUTResult CPUTButton::Resize(int width, int height)
{
    ...
    default:
        ASSERT(0,""); // todo: error! unknown state - using idle dimensions as a default
        safeWidth =  mSmallestLeftSizeIdle + mSmallestRightSizeIdle + 1;
        safeHeight =  mSmallestTopSizeIdle + mSmallestBottomSizeIdle + 1;
    }
  ...
  
    {
        ...
        // left
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 0*6, (float)mButtonDimensions.x, (float)mButtonDimensions.y, (float) mpButtonIdleSizeList[0].width, (float) mpButtonIdleSizeList[0].height, mpUVCoords_active[2*0], mpUVCoords_active[2*0+1], mColor );
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 1 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y + mSmallestTopSizeIdle, (float)mpButtonIdleSizeList[1].width, (float)middleHeight, mpUVCoords_active[2 * 1], mpUVCoords_active[2 * 1 + 1], mColor);
        mpButtonIdleSizeList[1].height = middleHeight;
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 2 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)mpButtonIdleSizeList[2].width, (float)mpButtonIdleSizeList[2].height, mpUVCoords_active[2 * 2], mpUVCoords_active[2 * 2 + 1], mColor);

        // middle
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 3 * 6, (float)mButtonDimensions.x + mSmallestLeftSizeIdle, (float)mButtonDimensions.y, (float)middleWidth, (float)mpButtonIdleSizeList[3].height, mpUVCoords_active[2 * 3], mpUVCoords_active[2 * 3 + 1], mColor);
        mpButtonIdleSizeList[3].width = middleWidth;
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 4 * 6, (float)mButtonDimensions.x + mSmallestLeftSizeIdle, (float)mButtonDimensions.y + mSmallestTopSizeIdle, (float)middleWidth, (float)middleHeight, mpUVCoords_active[2 * 4], mpUVCoords_active[2 * 4 + 1], mColor);
        mpButtonIdleSizeList[4].width = middleWidth;
        mpButtonIdleSizeList[4].height = middleHeight;
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 5 * 6, (float)mButtonDimensions.x + mSmallestLeftSizeIdle, (float)mButtonDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)middleWidth, (float)mpButtonIdleSizeList[5].height, mpUVCoords_active[2 * 5], mpUVCoords_active[2 * 5 + 1], mColor);
        mpButtonIdleSizeList[5].width = middleWidth;

        // right
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 6 * 6, (float)mButtonDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mButtonDimensions.y, (float)mpButtonIdleSizeList[6].width, (float)mpButtonIdleSizeList[6].height, mpUVCoords_active[2 * 6], mpUVCoords_active[2 * 6 + 1], mColor);
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 7 * 6, (float)mButtonDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mButtonDimensions.y + mSmallestTopSizeIdle, (float)mpButtonIdleSizeList[7].width, (float)middleHeight, mpUVCoords_active[2 * 7], mpUVCoords_active[2 * 7 + 1], mColor);
        mpButtonIdleSizeList[7].height = middleHeight;
        AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 8 * 6, (float)mButtonDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mButtonDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)mpButtonIdleSizeList[8].width, (float)mpButtonIdleSizeList[8].height, mpUVCoords_active[2 * 8], mpUVCoords_active[2 * 8 + 1], mColor);

        // Pressed button quads
        // left
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 0 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y, (float)mpButtonPressedSizeList[0].width, (float)mpButtonPressedSizeList[0].height, mpUVCoords_pressed[2 * 0], mpUVCoords_pressed[2 * 0 + 1], mColor);
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 1 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y + mSmallestTopSizePressed, (float)mpButtonPressedSizeList[1].width, (float)middleHeight, mpUVCoords_pressed[2 * 1], mpUVCoords_pressed[2 * 1 + 1], mColor);
        mpButtonPressedSizeList[1].height = middleHeight;
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 2 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y + mSmallestTopSizePressed + middleHeight, (float)mpButtonPressedSizeList[2].width, (float)mpButtonPressedSizeList[2].height, mpUVCoords_pressed[2 * 2], mpUVCoords_pressed[2 * 2 + 1], mColor);

        // middle
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 3 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed, (float)mButtonDimensions.y, (float)middleWidth, (float)mpButtonPressedSizeList[3].height, mpUVCoords_pressed[2 * 3], mpUVCoords_pressed[2 * 3 + 1], mColor);
        mpButtonPressedSizeList[3].width = middleWidth;
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 4 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed, (float)mButtonDimensions.y + mSmallestTopSizePressed, (float)middleWidth, (float)middleHeight, mpUVCoords_pressed[2 * 4], mpUVCoords_pressed[2 * 4 + 1], mColor);
        mpButtonPressedSizeList[4].width = middleWidth;
        mpButtonPressedSizeList[4].height = middleHeight;        
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 5 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed, (float)mButtonDimensions.y + mSmallestTopSizePressed + middleHeight, (float)middleWidth, (float)mpButtonPressedSizeList[5].height, mpUVCoords_pressed[2 * 5], mpUVCoords_pressed[2 * 5 + 1], mColor);
        mpButtonPressedSizeList[5].width = middleWidth;

        // right
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 6 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed + middleWidth, (float)mButtonDimensions.y, (float)mpButtonPressedSizeList[6].width, (float)mpButtonPressedSizeList[6].height, mpUVCoords_pressed[2 * 6], mpUVCoords_pressed[2 * 6 + 1], mColor);
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 7 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed + middleWidth, (float)mButtonDimensions.y + mSmallestTopSizePressed, (float)mpButtonPressedSizeList[7].width, (float)middleHeight, mpUVCoords_pressed[2 * 7], mpUVCoords_pressed[2 * 7 + 1], mColor);
        mpButtonPressedSizeList[7].height = middleHeight;
        AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, 8 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed + middleWidth, (float)mButtonDimensions.y + mSmallestTopSizePressed + middleHeight, (float)mpButtonPressedSizeList[8].width, (float)mpButtonPressedSizeList[8].height, mpUVCoords_pressed[2 * 8], mpUVCoords_pressed[2 * 8 + 1], mColor);


        // Disabled button quads
        // left
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 0 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y, (float)mpButtonDisabledSizeList[0].width, (float)mpButtonDisabledSizeList[0].height, mpUVCoords_disabled[2 * 0], mpUVCoords_disabled[2 * 0 + 1], mColor);
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 1 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y + mSmallestTopSizePressed, (float)mpButtonDisabledSizeList[1].width, (float)middleHeight, mpUVCoords_disabled[2 * 1], mpUVCoords_disabled[2 * 1 + 1], mColor);
        mpButtonDisabledSizeList[1].height = middleHeight;
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 2 * 6, (float)mButtonDimensions.x, (float)mButtonDimensions.y + mSmallestTopSizePressed + middleHeight, (float)mpButtonDisabledSizeList[2].width, (float)mpButtonDisabledSizeList[2].height, mpUVCoords_disabled[2 * 2], mpUVCoords_disabled[2 * 2 + 1], mColor);

        // middle
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 3 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed, (float)mButtonDimensions.y, (float)middleWidth, (float)mpButtonDisabledSizeList[3].height, mpUVCoords_disabled[2 * 3], mpUVCoords_disabled[2 * 3 + 1], mColor);
        mpButtonDisabledSizeList[3].width = middleWidth;
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 4 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed, (float)mButtonDimensions.y + mSmallestTopSizePressed, (float)middleWidth, (float)middleHeight, mpUVCoords_disabled[2 * 4], mpUVCoords_disabled[2 * 4 + 1], mColor);
        mpButtonDisabledSizeList[4].width = middleWidth;
        mpButtonDisabledSizeList[4].height = middleHeight;        
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 5 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed, (float)mButtonDimensions.y + mSmallestTopSizePressed + middleHeight, (float)middleWidth, (float)mpButtonDisabledSizeList[5].height, mpUVCoords_disabled[2 * 5], mpUVCoords_disabled[2 * 5 + 1], mColor);
        mpButtonDisabledSizeList[5].width = middleWidth;

        // right
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 6 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed + middleWidth, (float)mButtonDimensions.y, (float)mpButtonDisabledSizeList[6].width, (float)mpButtonDisabledSizeList[6].height, mpUVCoords_disabled[2 * 6], mpUVCoords_disabled[2 * 6 + 1], mColor);
        AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 7 * 6, (float)mButtonDimensions.x + mSmallestLeftSizePressed + middleWidth, (float)mButtonDimensions.y + mSmallestTopSizePressed, (float)mpButtonDisabledSizeList[7].width, (float)middleHeight, mpUVCoords_disabled[2 * 7], mpUVCoords_disabled[2 * 7 + 1], mColor);
