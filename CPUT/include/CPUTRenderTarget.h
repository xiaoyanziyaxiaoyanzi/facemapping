...
{
public:
	static CPUTRenderTargetColor * Create();
...
public:
	...
	ID3D11Texture2D           *GetColorTextureDX() { return mpColorTextureDX; }

	void                      CopyToTexture(CPUTTexture *texture);
    ID3D11RenderTargetView    *GetColorBufferView()         { return mpColorRenderTargetView; }
    ...
    CD3D11_TEXTURE2D_DESC      GetColorDesc()               { return mColorDesc; }
    ...
    void                      SaveRenderTargetToFile( CPUTRenderParameters &renderParams, std::string fileName, int format );
};
...
public:
	...
	static CPUTRenderTargetDepth* Create();
