...
)
{
    ...
    return CreateRenderTarget(
        ...
        width,
        height,
        (DXGI_FORMAT_UNKNOWN != colorFormat) ? colorFormat : mColorFormat,
        multiSampleCount ? multiSampleCount : mMultiSampleCount,
        mHasUav,
        true
    );
}
//-----------------------------------------------------------------------------
void CPUTRenderTargetColor::SaveRenderTargetToFile( CPUTRenderParameters &renderParams, std::string fileName, int format )
{
    ID3D11DeviceContext *pContext = CPUT_DX11::GetContext();
    ID3D11Resource *pResource = mpColorTextureDX;
    ...
    DirectX::SaveDDSTextureToFile(pContext, pResource, wstr);
    delete[] wstr;
}
...
)
{
    ...
    return CreateRenderTarget(
        ...
        width,
        height,
        (DXGI_FORMAT_UNKNOWN != depthFormat) ? depthFormat : mDepthFormat,
        multiSampleCount ? multiSampleCount : mMultiSampleCount,
        true
    );
}
...
D3D11_MAPPED_SUBRESOURCE CPUTRenderTargetColor::MapRenderTarget( CPUTRenderParameters &params, eCPUTMapType type, bool wait )
{
    ...
    if( !mpColorTextureDXStaging )
    {
        ...
        CPUTSetDebugName( mpColorTextureDXStaging, "Staging buffer" );
    }
    else
    {
...
//-----------------------------------------------------------------------------
CPUTSoftwareTexture *CPUTRenderTargetColor::GetSoftwareTexture(bool forceUpdate, bool updateIfDirty)
{
	if (forceUpdate || mpSoftwareTexture == NULL || (updateIfDirty && mSoftwareTextureDirty))
	{
		...
		D3D11_MAPPED_SUBRESOURCE desc = MapRenderTarget(params, CPUT_MAP_READ, true);
		mpSoftwareTexture->SetData(desc.pData, mWidth, mHeight, desc.RowPitch, mColorFormat);
		mSoftwareTextureDirty = false;
		UnmapRenderTarget(params);
	}
	return mpSoftwareTexture;
}

CPUTRenderTargetColor * CPUTRenderTargetColor::Create()
{
	return new CPUTRenderTargetColor();
}
...
//-----------------------------------------------------------------------------
void CPUTRenderTargetDepth::SaveRenderTargetToFile( CPUTRenderParameters &renderParams, std::string fileName, int format )
{
    ID3D11DeviceContext *pContext = CPUT_DX11::GetContext();
    ID3D11Resource *pResource = mpDepthTextureDX;
...
CPUTRenderTargetDepth* CPUTRenderTargetDepth::Create()
{
	return new CPUTRenderTargetDepth();
}
