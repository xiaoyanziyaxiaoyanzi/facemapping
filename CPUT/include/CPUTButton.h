...
{
public:
	static CPUTButton* Create(std::string const& ControlText, CPUTControlID id, CPUTFont* pFont);
    ...
    virtual void GetText(std::string &String);
...
private:
    CPUTButton(const std::string ControlText, CPUTControlID id, CPUTFont *pFont);
    CPUTButton(CPUTButton& copy); // don't allow copy construction
    CPUTButton& operator=(CPUTButton const& copy); // don't allow assignment
};
