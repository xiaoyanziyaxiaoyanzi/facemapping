...
void Menu_Scan::ActivationChanged(bool active)
{
	...
	if (active)
	{
		SetState(ScanState_ColorFeed );  //true则创建begin scan和skip scan的按钮，否则系统自动跳过脸部扫描过程
		if (!FaceScan_Init())
		{
...
{
	if (eventID == CPUT_EVENT_DOWN)  //eventID的值代表按下、点击等行为。如果eventID为CPUT_EVENT_DOWN，则判断controlID（ScanMenuIds）状态
	{
		switch (controlID)
		{
		...
		case ScanMenuIds_StopScanning:
		{
			...
			{
				std::string filename;
				CPUTFileSystem::CombinePath(GetUserDataDirectory(), "joe_sr300_1.obj", &filename);
				...
				gMenu_FaceMapping->LoadFace(filename.c_str());
				MenuController_PushMenu(gMenu_FaceMapping);

				//gMenu_FaceMapping->LoadFace(filename);
				//gMenu_FaceScanPreview->SetFaceScanMode(FaceScanPreviewMode_ApproveScan);
				////MenuController_PushMenu(gMenu_FaceMapping,false);
	...
	/*if (mScanState == ScanState_ScanComplete)
	{
		std::string filename;
		CPUTFileSystem::CombinePath(GetUserDataDirectory(), "joe_sr300_1.obj", &filename);
		gMenu_FaceScanPreview->LoadFaceObj(filename,true,true);
