...
//结束当前，开始下一个选择
static void FireActivationEvents(MenuBase *activeMenu, MenuBase *nextMenu)  //activeMenu不为空时，结束当前子类操作，nextmunu不为空时开始转向对应的menu键操作。都不为空就是结束当前操作转向下一个操作。
{
...
static MenuBase *GetActiveMenu()//获取当前正在执行的操作，静态函数
{
...
void MenuController_PushMenu( MenuBase *menu, bool defer )//defer为false则结束当前操作，执行push进来的操作
{
	if ( defer )
	{ 
	...
	else
	{
		...
		gController.MenuStack.push(menu);  //栈顶是正在执行的操作
	}
}
